...
// ── Hero animations ──────────────────────────────────────────────────────────
// Uses fromTo() to be EXPLICIT about start AND end states.
// This avoids the bug where gsap.from() reads CSS opacity:0 as the target end.
function initHeroAnimations() {
  ...
  if (!greeting) return; // not on hero page
...
// Uses fromTo() — avoids immediateRender opacity:0 staying when trigger never fires.
function initScrollAnimations() {
  // Fade-up: elements that have CSS opacity:0 via .gsap-reveal
  gsap.utils.toArray('.gsap-reveal').forEach(el => {
    // Skip hero elements (they are handled by initHeroAnimations)
  ...
  // Staggered children — immediateRender:false keeps elements VISIBLE until trigger fires.
  // This prevents the "invisible forever if trigger never fires" bug.
...
// ── Init all ──────────────────────────────────────────────────────────────────
function initAll() {
  // Kill previous ScrollTrigger instances before re-init
  ...
  // Refresh after a tick so layout is stable
...
if (document.readyState === 'loading') {
  ...
  // DOM already ready (script ran after parse)
